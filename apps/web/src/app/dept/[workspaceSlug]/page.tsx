    ...
    <>
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
          ...
          <Panel title="Pipeline snapshot" description="Candidates closest to a decision — open pipeline for the full board.">
            <ul className="space-y-3 text-sm">
              {pipelinePreview.map((row) => (
                <li key={row.id} className="flex flex-col gap-1 rounded-md border border-slate-100 p-3 dark:border-slate-800/80">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="font-medium text-slate-900 dark:text-slate-50">{row.name}</span>
                    <Badge variant="outline">{row.status}</Badge>
                  </div>
                  <div className="text-xs text-slate-500 dark:text-slate-400">{row.auditionTitle}</div>
          ...
          <Panel title="Auditions" description="Templates and live auditions your team is running.">
            <ul className="space-y-3 text-sm">
              {auditionsPreview.map((a) => (
                <li key={a.id} className="rounded-md border border-slate-100 p-3 dark:border-slate-800/80">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="font-medium text-slate-900 dark:text-slate-50">{a.title}</span>
        ...
        <Panel title="Next actions" description="High-leverage shortcuts for this workspace.">
          <ol className="list-decimal space-y-3 pl-4 text-sm text-slate-700 dark:text-slate-200">
